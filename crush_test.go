...
func TestCrushAddAndPlaceBestEffort(t *testing.T) {
	peerId1 := uint64(7412850045943536150)
	m1 := make(map[string]string)
	m1[serfMDKeyDataCenter] = "dc1"
	m1[serfMDKeyRack] = "rack1"
	...
	dsId, err := parseIdFromString("01CG0986V8JPXVER781B4K9CQ0")
	assert.Nil(t, err)
	wv := crush.placeBestEffort(dsId, 2, 1, 2)
	...
	pickedPeer1 := wv.intermediateResults[2][0].(uint64)
	pickedPeer2 := wv.intermediateResults[2][2].(uint64)
	assert.Equal(t, peerId4, pickedPeer1)
	assert.Equal(t, nodeId2, pickedPeer2)

	// may contain nils as not all slots might have a peer to them
	assert.Nil(t, wv.intermediateResults[2][1])
	assert.Nil(t, wv.intermediateResults[2][3])
}
